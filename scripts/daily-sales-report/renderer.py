...
from state_map import STATES, STATE_NAME_TO_CODE, color_for_pct
...
.sc-plan { font-size: 12px; color: var(--gray-500); font-weight: 500; }
...
.sc-badge { padding: 4px 9px; border-radius: 6px; font-size: 11px; font-weight: 700; display: inline-flex; align-items: center; gap: 3px; line-height: 1.3; }
...
def _render_state_map(state_data):
    ...
    svg_paths = []
    svg_paths.append('<rect width="520" height="310" fill="#f1f5f9" rx="6"/>')
    for s in STATES:
        ...
        svg_paths.append(f'<path d="{s["d"]}" fill="{fill}" stroke="#fff" stroke-width="1.5"><title>{escape(title)}</title></path>')
        if s.get("no_label"):
            continue
        if s.get("external_label"):
            # RI: external label with connector
            svg_paths.append('<line x1="347" y1="197" x2="375" y2="190" stroke="#6b7280" stroke-width="0.8"/>')
            color = "#16a34a" if vsP >= 0 else "#dc2626"
            svg_paths.append(f'<text x="390" y="188" text-anchor="middle" font-size="8" font-weight="700" fill="{color}" font-family="Arial,sans-serif">{s["code"]} {_fmt_pct(vsP,1)}</text>')
            continue
        if s.get("combined_label"):
            x, y = s["label_xy"]
            svg_paths.append(f'<text x="{x}" y="{y}" text-anchor="middle" font-size="8.5" font-weight="700" fill="{s["label_fill"]}" font-family="Arial,sans-serif">{s["code"]}  {_fmt_pct(vsP,1)}</text>')
            continue
        lx, ly = s["label_xy"]; px, py = s["pct_xy"]
        svg_paths.append(f'<text x="{lx}" y="{ly}" text-anchor="middle" font-size="8.5" font-weight="700" fill="{s["label_fill"]}" font-family="Arial,sans-serif">{s["code"]}</text>')
        svg_paths.append(f'<text x="{px}" y="{py}" text-anchor="middle" font-size="8" fill="{s["pct_fill"]}" font-family="Arial,sans-serif">{_fmt_pct(vsP,1)}</text>')

    # Legend
    svg_paths.append('''<defs><linearGradient id="legGrad" x1="0" x2="1" y1="0" y2="0"><stop offset="0%" stop-color="#dc3532"/><stop offset="50%" stop-color="#f5c832"/><stop offset="100%" stop-color="#2cab45"/></linearGradient></defs>
<rect x="12" y="282" width="120" height="10" rx="3" fill="url(#legGrad)"/>
<text x="12" y="278" font-size="7.5" fill="#6b7280" font-family="Arial,sans-serif">-35%</text>
<text x="66" y="278" text-anchor="middle" font-size="7.5" fill="#6b7280" font-family="Arial,sans-serif">0%</text>
<text x="132" y="278" text-anchor="end" font-size="7.5" fill="#6b7280" font-family="Arial,sans-serif">+25%</text>
<text x="66" y="307" text-anchor="middle" font-size="7" fill="#9ca3af" font-family="Arial,sans-serif">vs Plan %</text>''')
